...
export class JobsService {
  ...
  async create(createJobDto: CreateJobDto, user: IUser) {
    const result = await this.jobModel.create({ ...createJobDto, isActive: true,  createdBy: user._id })
  ...
  async update(id: string, updateJobDto: UpdateJobDto, userId: string) {
    const result = await this.jobModel.updateOne({ _id: id }, { ...updateJobDto, updatedBy: new mongoose.Types.ObjectId(userId) })
    return result
  }
